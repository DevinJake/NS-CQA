...
if __name__ == "__main__":
    ...
    sys.argv = ['data_test_RL_webqsp.py',
                '-m=epoch_000_0.838_0.000.dat',
                '-p=rl', '--n=rl_cher', '--att=0', '-w2v=300', '--lstm=1']
    ...
    PREDICT_PATH = '../data/webqsp_saves/' + str(args.name) + '/' + str(args.pred) + '_predict.actions'
    fwPredict = open(PREDICT_PATH, 'w', encoding="UTF-8")
    REFER_PATH = '../data/webqsp_saves/' + str(args.name) + '/' + str(args.pred) + '_refer.actions'
    ...
    model_path = '../data/webqsp_saves/' + str(args.name) + '/' + str(args.model)
